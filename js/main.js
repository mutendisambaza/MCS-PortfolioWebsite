...
$(document).ready(function () {
  ...
  cardWraps.each(function () {
    const card = $(this).find('.card'); // Ensure we're targeting the card
    const borderWrap = $(this).find('.border-wrap');
    let cardBounds = null;

    // Attach hover event directly to the card
    card.on('mouseenter', function () {
      cardBounds = card[0].getBoundingClientRect(); // Get exact card bounds
      $(document).on('mousemove', rotateToMouse);
    });

    card.on('mouseleave', function () {
      $(document).off('mousemove', rotateToMouse);
      card.css('transform', ''); // Reset transformation on mouse leave
      borderWrap.css('background-image', ''); // Reset background effect
    });
